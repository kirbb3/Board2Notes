...
2. **Fusion stage (not yet built):** merge transcript-engine JSON
   (timestamps, ★ flags, emphasis) with board content into one document —
   board math as the spine, spoken explanations as connective prose,
   ★ callout boxes. The LLM pass should also fix ASR errors using context
   (e.g. "minimum wage spanning tree" → "minimum weight").
